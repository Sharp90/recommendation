...
# mood_w : 0(bad) ~ 1(good) weight
def MENU_e(D, mood_w, get_mean, get_observation, S=1., lamb=1., delta=0.01, p=1.5, c=1.):
    T, K, d=D.shape
    ...
    for n in range(N):
        ...
        Dt = np.zeros((K, 2 * d))
        for i in range(K):
            Dt[i, :d] = D[n][i] * mood_w[K * n + i]
            Dt[i, d:] = D[n][i] * (1 - mood_w)[K * n + i]
        Rt = get_mean(Dt[:, :d], n)
